...
#
    #while true; do
        for file in $(ls /home/tomaking/.config/fastfetch/asciiart/*.txt | sort -V); do
            clear
            paste <(cat "$file") <(fastfetch --config ~/.config/fastfetch/configII.jsonc)  # Zeigt ASCII links und Fastfetch rechts
